...
| B19 | Follow-up from B10's audit: ~12 skills invent Ronin codenames (Kanroku #09, Sadaemon #21, Kyūdayū #10, Magokurō #11, etc.) at roster slots `docs/RONIN_ROSTER.md` marks unassigned — same class of error A23 was written to stop. Needs a decision (remove the invented names vs. formally assign them in `roster.rs`), not just a mechanical fix | every affected skill's codename references resolved against `docs/RONIN_ROSTER.md`, zero contradictions |
