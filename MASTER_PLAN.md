...
| A11 | Versioned Postman v2.1 non-mutating local contract for loopback web/control/Node/Brain surfaces; external workspace reference remains unverified and the local collection provides no external execution authority. | `postman/` (15 allowlisted read/pure-compute/dry-run requests); external collection `e6b5fcae…` accessibility **UNVERIFIED** |
