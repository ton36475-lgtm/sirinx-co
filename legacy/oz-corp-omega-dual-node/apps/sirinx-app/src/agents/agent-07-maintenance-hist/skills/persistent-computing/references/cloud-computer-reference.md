...
Monthly billing. All plans include Ubuntu Server 24.04 LTS 64-bit, independent public IP, 1000 Mbps inbound bandwidth, and 10 GB storage.
...
| **Outbound traffic** | 3 GB | 10 GB | 25 GB |
| **Outbound bandwidth** | 10 Mbps | 50 Mbps | 100 Mbps |
...
- If you have verified that the attached machine is already on the top tier (Advanced) and still cannot meet the workload, do not suggest an upgrade; discuss alternatives with the user instead.
...
- When deploying services, you MUST open the required ports via `sudo ufw allow <port>`. Never expose unprotected services (no-auth databases, admin panels without passwords) directly to the internet.
