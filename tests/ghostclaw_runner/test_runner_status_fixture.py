...
class A2A2ARunnerStatusFixtureTest(unittest.TestCase):
    ...
    def test_worker_followup_implementation_packet_is_codex_owned_scope(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ghostclaw-worker-followup-packet-") as tmp:
            runtime = Path(tmp) / "runtime"
            lane_path = Path(tmp) / "lane.json"
            fixture_path = Path(tmp) / "packet.json"
                ...
                json.dumps(
                    {
                        ...
                        "lane": {
                            "laneId": "LANE_A2A2A_WORKER_FEEDBACK_CONSUMPTION-abc123",
            ...
            exit_code = a2a_worker_followup_implementation_packet.main(
                [
                    "--lane-path",
                    str(lane_path),
                    "--runtime-root",
            ...
            self.assertEqual(exit_code, 0)
            fixture = json.loads(fixture_path.read_text(encoding="utf-8"))
            self.assertEqual(fixture["summary"]["status"], "ready_for_codex_scoped_work")
            self.assertTrue(fixture["summary"]["executionAllowed"])
            self.assertFalse(fixture["summary"]["providerCallsAllowed"])
            self.assertFalse(fixture["summary"]["workerDirectEditsAllowed"])
            self.assertFalse(fixture["summary"]["gitAddDotAllowed"])
    ...
    def test_next_scoped_coding_packet_skips_completed_backlog_item(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ghostclaw-next-scoped-coding-skip-") as tmp:
            runtime = Path(tmp) / "runtime"
            start_packet = {
                "summary": {
                    "status": "ready_for_team_coding",
                ...
                "priorityQueue": [
                    ...
                    },
                    {
                        ...
                        "owner": "codex",
                        "priority": 1,
                        "task": "Add optional UI panel in Mission Control to browse local campaign packs.",
                        "nextAction": "Codex may inspect and prepare scoped local edits after validation.",
                    },
                ],
            }
            completed_outcome = {
                "summary": {
                    "status": "packet_completed",
                    "selectedBacklogId": "BACKLOG-092",
                    "selectedPacketId": "SCOPED-CODING-420bddf8b5",
                    "validationStatus": "passed",
                    "validationFailed": 0,
                }
            }

            fixture = a2a_next_scoped_coding_packet.build_packet(
                start_packet,
                runtime,
                completed_outcomes=[completed_outcome],
            )

            self.assertEqual(fixture["summary"]["status"], "ready_for_scoped_coding_packet")
            self.assertEqual(fixture["summary"]["selectedBacklogId"], "BACKLOG-096")
            self.assertEqual(fixture["packet"]["selectedBacklogId"], "BACKLOG-096")
            self.assertEqual(fixture["packet"]["task"], "Add optional UI panel in Mission Control to browse local campaign packs.")
            self.assertIn("BACKLOG-092", fixture["completedBacklogIds"])
            self.assertIn("scripts/a2a/a2a_campaign_pack_browser_status.py", fixture["packet"]["plannedFiles"])
            self.assertIn("docs/marketing_automation/", fixture["packet"]["allowedPaths"])
            self.assertIn(
                ...
                fixture["packet"]["validationCommands"],
            )
