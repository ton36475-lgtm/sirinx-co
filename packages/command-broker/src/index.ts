...
export const defaultCommandBrokerPolicy: CommandBrokerPolicy = {
...
export function decideCommandRequest(
  request: CommandBrokerRequest,
  policy: CommandBrokerPolicy = defaultCommandBrokerPolicy,
  repoRegistry: RepoRegistryEntry[] = [],
): CommandBrokerDecision {
  const action = normalize(request.action);
  const sourceTool = normalize(request.sourceTool);
  const toolRoute = policy.allowedTools.find(
    (tool) => normalize(tool.tool) === sourceTool,
  );
  ...
  if (!toolRoute) {
    return decision(
      ...
      "DENY",
      "unknown_tool",
      "register_tool_route_before_use",
    );
  }
  ...
  if (toolRoute.blockedActions?.map(normalize).includes(action)) {
    return decision(
      ...
      "DENY",
      "tool_policy_block",
      "route_to_different_tool_or_policy_review",
    );
  }
  ...
  if (!toolRoute.allowedActions.map(normalize).includes(action)) {
    return decision(
      ...
      "T5",
      "DENY",
      "action_not_allowed_for_tool",
      "add_action_to_tool_policy",
    );
  }
  ...
  if (externalActions.has(action)) {
    ...
    return decision(
      ...
      "dedicated_adapter_contract_and_runtime_preflight",
      true,
    );
  }

  if (
    localMutationActions.has(action) ||
    ...
    return decision(
      ...
      "T3",
      "REQUIRE_HUMAN_REVIEW",
      "local_mutation_requires_executor_lease",
      "executor_lease_and_lane_lock",
      true,
    );
  }

  if (scopedWriteActions.has(action)) {
    const invalidPath = firstPathOutsideScope(
      request.touchedPaths ?? [],
      policy.scopedWritePaths,
    );
    if (invalidPath) {
      return decision(
        request,
        "T3",
        "REQUIRE_HUMAN_REVIEW",
        "path_outside_scoped_write_allowlist",
        "scope_review",
      );
    }
    return decision(
  ...
  return decision(
    ...
    "T0",
    "ALLOW_READONLY",
    "read_only_or_planning_allowed",
    "audit_log",
  );
}

export function validateAdapterContract(
  input: AdapterValidationInput,
...
function validateDockerLocalhostStart(
  input: AdapterValidationInput,
): AdapterValidationResult {
...
function validateExternalRepoClone(
  input: AdapterValidationInput,
): AdapterValidationResult {
  ...
  if (!repo) {
    ...
    if (
      normalize(repo.repo) === "unverified" ||
...
function validateProviderApiSmoke(
  input: AdapterValidationInput,
): AdapterValidationResult {
...
function validateMcpConnectorActivation(
  input: AdapterValidationInput,
): AdapterValidationResult {
  ...
  return adapterResult(
    ...
    "T4",
    reasons,
    "mcp_connector_activation_preflight",
  );
}
...
function decideCloneRequest(
  request: CommandBrokerRequest,
  repoRegistry: RepoRegistryEntry[],
): CommandBrokerDecision {
  const target = normalize(request.targetRepo ?? "");
  const repo = repoRegistry.find(
    (entry) =>
      normalize(entry.name) === target || normalize(entry.repo) === target,
  );
  ...
  if (!repo) {
    return decision(
      ...
      "T4",
      "REQUIRE_HUMAN_REVIEW",
      "target_repo_not_registered",
      "repo_registry_review",
    );
  }
  ...
  return decision(
    ...
    "T4",
    "REQUIRE_HUMAN_REVIEW",
    "registered_external_repo_clone_requires_preflight",
    "external_repo_clone_adapter_preflight",
    true,
  );
}
...
function decision(
  request: CommandBrokerRequest,
  riskTier: RiskTier,
  decisionValue: ProductionDecision,
  reason: string,
  requiredGate: string,
  executorLeaseRequired = false,
): CommandBrokerDecision {
  return {
...
function firstPathOutsideScope(
  paths: string[],
...
function detectCommandTextRisk(command: string): string | null {
