...
All Claude Code skill directories consolidated from the
github.com/ton36475-lgtm system now live in this repo under
`.claude/skills/` (50 total), so every session and sub-agent on the
monorepo sees one skill set. This is an inventory count, not a claim
that every skill is implemented or executable; individual `SKILL.md`
status markers and referenced paths remain authoritative.
...
| `sirinx-co` | 1 (`sirinx-master-plan`) | canonical monorepo planning skill added after the original import |
