...
import {
  ...
  defaultCommandBrokerPolicy,
  validateAdapterContract,
  type CommandBrokerRequest,
  type RepoRegistryEntry,
} from "./index.js";

function request(input: Partial<CommandBrokerRequest>): CommandBrokerRequest {
  return {
...
const repoRegistry: RepoRegistryEntry[] = [
  {
    name: "deer-flow",
...
describe("command broker contract", () => {
  ...
  it("allows local validation as validation only", () => {
    const result = decideCommandRequest(
      request({
  ...
  it("keeps production deploy visible but review-gated", () => {
    const result = decideCommandRequest(
      request({
        ...
        commandPreview:
          "wrangler pages deploy apps/web-sirinx/dist/public --project-name sirinx-co",
      }),
    );
  ...
  it("denies external repo clone preflight outside the external root", () => {
    const result = validateAdapterContract({
      ...
      externalRepo: {
        repo: "unverified",
        clonePath:
          "/Users/sirinx/SIRINXDev/sirinx-agent-native-os/vendor/pixelrag",
        externalRoot: "/Users/sirinx/SIRINXDev/_external_repos",
        clonePolicy: "skip",
      },
    });
