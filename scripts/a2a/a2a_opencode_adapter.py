#!/usr/bin/env python3
"""Create OpenCode A2A command-plan artifacts."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create an OpenCode scoped executor command plan")
...
def main() -> int:
    ...
    version = safe_run(["opencode", "--version"], timeout=8)
    command_preview = ["opencode", "<scoped-task>", "--repo", args.repo_path, "--lane", args.lane]
    artifact = {
        "created_at": now_iso(),
        "adapter": "opencode",
        ...
        "note": "No OpenCode provider task was executed by this adapter in dry-run mode.",
    }
    out = runtime_path("artifacts", f"{task_id('OPENCODE')}-{args.lane}.json")
